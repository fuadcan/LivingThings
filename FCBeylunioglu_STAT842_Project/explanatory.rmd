...
After filtering out non-intersecting songs we can compare distributions of features based on their genres. Above quantile plots show how different the features distribute. The most salient difference is seen in speechiness that yields very intuitive results, accordingly rock songs tend to have less speechiness score than hip-hop. Other major differences are seen for instrumentalness, danceability and valence where rock songs are more instrumental, less danceable and darker (having negative valences) compared to hip-hop. 

To see if these features have enough correlation with genres we present the below eikosograms. For below displays we clustered each estimation of features into four categories falling in between 5 equidistant benchmarks (e.g. if the range is [0,1], the benchmarks are 0,.25,.50,.75,1). As displays show, the strongest relations are between danceability, speechiness, instrumentalness and valence. On the other hand liveness and tempo levels are almost independent from genre classes whereas the remaining two features do not have strong enough correlations compared to others mentioned.
